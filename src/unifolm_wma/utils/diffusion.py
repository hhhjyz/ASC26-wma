...
class _SinusoidalFreqStore:
    ...
    def fetch(cls, dim_half, period_val, dev):
        ...
        if key in cls._data:
            return cls._data[key].clone()
        
        # 计算频率
        freq = torch.exp(
            -math.log(period_val) *
            torch.arange(start=0, end=dim_half, dtype=torch.float32, device=dev) /
            dim_half
        )
        
        # 缓存管理
        if len(cls._data) >= cls._limit:
            cls._data.clear()
...
def timestep_embedding(timesteps, dim, max_period=10000, repeat_only=False):
    ...
    if not repeat_only:
        half = dim // 2
        # 优化: 使用缓存的频率
        freqs = _SinusoidalFreqStore.fetch(half, max_period, timesteps.device)
