...
class WMAModel(nn.Module):
    ...
    def forward(self,
        ...
        predict_action_head = kwargs.pop("predict_action_head", True)
        predict_state_head = kwargs.pop("predict_state_head", True)

        if not self.base_model_gen_only:
            ba, _, _ = x_action.shape
            if predict_action_head:
