...
class DDIMSampler(object):
    ...
    def sample(
        ...
        schedule_cache_key = (S, timestep_spacing, eta)
        if self._schedule_cache_key != schedule_cache_key:
            self.make_schedule(ddim_num_steps=S,
                               ddim_discretize=timestep_spacing,
                               ddim_eta=eta,
                               verbose=schedule_verbose)
            self._schedule_cache_key = schedule_cache_key
    ...
    def ddim_sampling(self,
        ...
        for i, step in enumerate(iterator):
            ...
            outs = self.p_sample_ddim(
                ...
                x0=x0,
                fs=fs,
                obs_encoder_cache=obs_encoder_cache,
                guidance_rescale=guidance_rescale,
                **kwargs)
    ...
    def p_sample_ddim(self,
        ...
        with torch.autocast(device_type='cuda', enabled=False):
            ...
            model_output_action = model_output_action.float()
            model_output_state = model_output_state.float()

            if self.model.parameterization == "v":
                e_t = self.model.predict_eps_from_z_and_v(
                    x_fp32, t, model_output_fp32)
            else:
                e_t = model_output_fp32

            if score_corrector is not None:
                assert self.model.parameterization == "eps", 'not implemented'
                e_t = score_corrector.modify_score(self.model, e_t, x_fp32, t,
                                                   c, **corrector_kwargs)
            ...
            if self.model.parameterization != "v":
                pred_x0 = (x_fp32 - sqrt_one_minus_at * e_t) / a_t.sqrt()
            else:
