...
class ConditionalUnet1D(nn.Module):
    ...
    def forward(self,
                ...
                **kwargs):
        """
        Conditional UNet1D forward pass for action/state prediction.
        
        Args:
            sample: (B,T,input_dim) - noisy action/state sequence
            timestep: (B,) or int - diffusion timestep
            imagen_cond: list of hidden features from video generation unet
            cond: tuple of (image, agent_pos) conditioning tensors
                - image: (B, 3, To, h, w)
                - agent_pos: (B, Ta, d)
        
        Returns:
            output: (B,T,input_dim) - predicted noise/signal
        """
        ...
        # Check for cached encoder output
        encoder_cache = kwargs.get('obs_encoder_cache', None)
        cache_key = None
        global_cond = None
        
        if encoder_cache is not None and not self.training:
            # Build cache key from tensor properties
            cache_key = (
                id(self),
                img_cond.data_ptr(),
                tuple(img_cond.shape),
                pos_cond.data_ptr(),
            ...
            global_cond = encoder_cache.get(cache_key)

        if global_cond is None:
            # Compute encoder output
            encoder_input = {
                'image': rearrange(img_cond.permute(0, 2, 1, 3, 4), 'b t c h w -> (b t) c h w'),
                'agent_pos': rearrange(pos_cond, 'b t d -> (b t) d')
            }
            global_cond = self.obs_encoder(encoder_input)
            # Store in cache if available
            if cache_key is not None:
                encoder_cache[cache_key] = global_cond
