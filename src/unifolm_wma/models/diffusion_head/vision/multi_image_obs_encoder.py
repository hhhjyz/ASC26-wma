...
class GPUNormalize(nn.Module):
    ...
    Registers mean/std as buffers so they automatically move with the model,
    avoiding repeated CPU-GPU transfers during forward pass.
    """
    ...
    def forward(self, x: Tensor) -> Tensor:
        if not self.inplace:
            x = x.clone()
        # Reshape mean/std for broadcasting if needed
        m = self.mean.view(-1, 1, 1) if self.mean.ndim == 1 else self.mean
        s = self.std.view(-1, 1, 1) if self.std.ndim == 1 else self.std
        return x.sub_(m).div_(s)
    
    def extra_repr(self) -> str:
        return f'mean={self.mean.tolist()}, std={self.std.tolist()}, inplace={self.inplace}'
