...
class SpatialSoftmax(ConvBase):
    ...
    def forward(self, feature):
        ...
        优化: 使用FP32计算softmax和期望值，避免混合精度下的数值溢出
        """
        ...
        # 优化: 转换为FP32进行softmax计算，避免数值溢出
        orig_dtype = feature.dtype
        feature_f32 = feature.reshape(-1, self._in_h * self._in_w).float()
        temp_f32 = self.temperature.float()
        
        # 2d softmax normalization (在FP32下计算)
        attention = F.softmax(feature_f32 / temp_f32, dim=-1)
        
        # 坐标计算也使用FP32
        pos_x_f32 = self.pos_x.float()
        pos_y_f32 = self.pos_y.float()
        
        # [1, H * W] x [B * K, H * W] -> [B * K, 1] for spatial coordinate mean in x and y dimensions
        expected_x = torch.sum(pos_x_f32 * attention, dim=1, keepdim=True)
        expected_y = torch.sum(pos_y_f32 * attention, dim=1, keepdim=True)
        # stack to [B * K, 2]
        ...
        if self.output_variance:
            ...
            expected_xx = torch.sum(pos_x_f32 * pos_x_f32 * attention,
                                    dim=1,
                                    keepdim=True)
            expected_yy = torch.sum(pos_y_f32 * pos_y_f32 * attention,
                                    dim=1,
                                    keepdim=True)
            expected_xy_cov = torch.sum(pos_x_f32 * pos_y_f32 * attention,
                                    dim=1,
                                    keepdim=True)
